    ...
    while let Some(child) = children.next() {
        if child.kind() == SyntaxKind::TABLE_HEADER {
            ...
            if child.text() == "[dependencies]" || child.text() == "[dev-dependencies]" {
                let section_children = get_section_children(&mut children);
                sort_nodes(&node, section_children, &|left, right| {
...
fn sort_nodes(
    ...
    cmp: &impl Fn(&SyntaxNode, &SyntaxNode) -> Ordering,
) {
    ...
    let children: Vec<_> = children
        .into_iter()
        .map(NodeWithLeadingTrivia::from)
        .collect();
...
impl NodeWithLeadingTrivia {
    ...
    pub fn start_index(&self) -> usize {
        self.trivia
            .first()
            .map(|t| t.index())
            .unwrap_or_else(|| self.node.index())
    }
...
fn sort_children(
    children: Vec<NodeWithLeadingTrivia>,
    cmp: &impl Fn(&SyntaxNode, &SyntaxNode) -> Ordering,
) -> Vec<NodeWithLeadingTrivia> {
...
fn sort_group(
    child_group: &mut Vec<NodeWithLeadingTrivia>,
    cmp: &impl Fn(&SyntaxNode, &SyntaxNode) -> Ordering,
) {
    ...
    let first_item_trivia_to_move = {
        ...
        let trivia = first_item
            .trivia
            .drain(..trivia_remove_end)
            .collect::<Vec<_>>();
        first_item.trivia.splice(0..0, group_trivia);
        trivia
    };
    let previous_first_item = child_group
        .iter_mut()
        .find(|t| t.node.index() == previous_first_item_index)
        .unwrap();
